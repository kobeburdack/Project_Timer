...
int L1G = A0, L1R = A1, L2G = A2, L2O = 10, L2R = 9; //defining leds
...
public:
  ...
  bool checkCom() //check the communication with the sensor
  {
    if (analogRead(port) >= 800)
    {
      Serial.print("Error sensor port ");
      Serial.print(port);
...
void loop()
{
  ...
  if (digitalRead(switches[3]) == LOW)
  {
    ...
    while (T1.getTimeCountDown() > 0 && R1.getState() == false) //if the counter is stopped or the first sensor detected a activity
    {
      TimeSeparate(T1.getTimeCountDown());  //display the time of the countdown timer
      D1.checkLedState(); //check te state of the led's
    }

    D1.startCounter1(); //start the firts counter
    R1.stopCom(); //stop the communication with sensor 1
    R2.startCom();  //start the communication with sensor 2
    D1.checkLedState(); //check te state of the led's

    while (R2.getState() == false)  //wait until sensor 2 detects something
    {
      TimeSeparate(D1.getTime1());  //display the time of the timer
    }
    D1.stopCounter1();  //stop counter 1
    D1.checkLedState(); //check te state of the led's

    if (digitalRead(switches[1]) == LOW)  //if the third sensor is activated do the following
    {
      D1.startCounter2(); //start the 2nd counter
      R2.stopCom(); //stop the communication with sensor 2
      R3.startCom();  //start the communication with sensor 3
      D1.checkLedState(); //check te state of the led's
      while (R3.getState() == false) //wait until sensor 3 detects something
      {
        TimeSeparate(D1.getTime2()); //display the time of the timer
      }
      D1.stopCounter2(); //stop counter 2
      D1.checkLedState(); //check te state of the led's
      R3.stopCom();       //stop the communication with sensor 3
    }
    else
    {
      R2.stopCom(); //stop the communication with sensor 2
    }
    D1.displayTime(); //show the time's of the counters and the total time
    delay(1000);
  }
}
